        ...
        async start(controller) {
            try {
                ...
                if (plan.mode === ExecutionMode.REACT_AGENT) {
                    // Agent模式：动态导入Agent工厂（避免循环依赖），创建Agent并启动流式输出
                    const { createAgent } = await import('@/lib/agents/agent-factory');
                    const agent = createAgent(ExecutionMode.REACT_AGENT);
                    // 以流式模式运行Agent，获取updates（状态更新）和messages（逐token输出）
                    const gen = await agent.stream(
                        { userQuestion: plan.rewrittenQuestion, longTermSummary: plan.longTermSummary, recentTranscript: plan.recentTranscript },
                        { streamMode: ['updates', 'messages'] }
                    );
                    for await (const [mode, update] of gen) {
                        // 仅处理消息模式的更新：提取文本内容并通过SSE推送给前端
                        if (mode === 'messages') {
                            const m = Array.isArray(update) ? update[0] : update;
                    ...
                    await conversationManager.completeExchange({
                        conversationId, exchangeId, answer: fullAnswer,
